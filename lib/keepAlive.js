...
function startKeepAliveServer() {
  ...
  }).listen(port, () => logInfo(`Serveur de santé HTTP à l'écoute sur le port ${port}`));
}
...
async function handleJoinGuild(req, res) {
  try {
    ...
    logError('join-guild: échec API Discord', discordRes.status, errText);
    res.writeHead(502, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ error: 'Échec de l\'ajout au serveur Discord' }));
  } catch (e) {
    logError('join-guild:', e.message);
    res.writeHead(500, { 'Content-Type': 'application/json' });
