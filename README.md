...
- **Logs Discord** — tous les logs du bot (infos + erreurs) *et* le journal du serveur (arrivées, départs, bannissements, messages modifiés/supprimés) sont relayés dans un salon dédié, en plus de la console/des logs Render.
...
4. Toujours dans **Bot** → section **Privileged Gateway Intents**, active **"Message Content Intent"** (relais de traduction + journal des messages modifiés/supprimés) et **"Server Members Intent"** (journal des arrivées/départs de membres).
